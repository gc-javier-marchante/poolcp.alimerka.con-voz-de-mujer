    ...
    if (elements && elements.length > 0) {
        for (var i = 0, len = elements.length; i < len; i++) {
            new KTImageInput(elements[i], {
                clear_on_init: elements[i].getAttribute('data-kt-image-input-preloaded') !== 'true'
            });
        }
    }
}
...
var KTUtil = function() {
    var resizeHandlers = [];
    var domAddedTimeout = null;
    var first_dom_load = true;
    /**
    ...
    return {
        ...
        onDomAdd: [],
        domAdded: function (do_not_defer) {
            clearTimeout(domAddedTimeout);
        
            if (first_dom_load) {
                do_not_defer = true;
                first_dom_load = false;
            }
        
            if (!do_not_defer) {
                domAddedTimeout = setTimeout(function () {
                    KTUtil.domAdded(true);
                }, 10);
        
                return;
            }
        
            for (let i = 0; i < KTUtil.onDomAdd.length; i++) {
                let callback = KTUtil.onDomAdd[i];
                ...
                if (typeof (callback) == 'function') {
                    ...
                    KTUtil.onDomAdd[i] = callback;
                }
        
                if (!callback._executions) {
                    callback._executions = 0;
                }
        
                if (callback.max == 0 || callback.max > callback._executions) {
                    callback._executions++;
                    callback.callback();
                }
            }
        }
